...
def customCNN(cnn_in, is_training, drop_rate):
    ...
    net = layers.conv2d(inputs=cnn_in, filters=32, kernel_size=5, padding='same')
    ...
    net = layers.conv2d(inputs=net, filters=32, kernel_size=5, padding='same')
    net = layers.batch_normalization(inputs=net, training=is_training)
    net = nn.relu(net)
    net = layers.dropout(inputs=net, rate=drop_rate, training=is_training)

    net = layers.conv2d(inputs=net, filters=32, kernel_size=5, strides=2, padding='same')
    net = layers.batch_normalization(inputs=net, training=is_training)
    net = nn.relu(net)
    net = layers.dropout(inputs=net, rate=drop_rate, training=is_training)

    net = layers.conv2d(inputs=net, filters=64, kernel_size=5, padding='same')
    net = layers.batch_normalization(inputs=net, training=is_training)
    net = nn.relu(net)
    net = layers.dropout(inputs=net, rate=drop_rate, training=is_training)
